...
export function OffersTable() {
  ...
  // Derived state
  const statuses = React.useMemo(() => Array.from(new Set(offers.map((o) => o.status))), []);
  const agents = React.useMemo(() => Array.from(new Set(offers.map((o) => o.agent))), []);
  ...
  const filteredOffers = React.useMemo(() => {
    return offers.filter((offer) => {
      ...
      const matchesAgent = agentFilter === "all" || offer.agent === agentFilter;
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50 hover:bg-muted/50">
              ...
              <TableHead className="min-w-[90px] font-medium text-muted-foreground text-xs sm:text-sm">Amount</TableHead>
            ...
            {paginatedOffers.length === 0 ? (
              ...
              paginatedOffers.map((offer, index) => (
                <TableRow key={offer.id}>
                  ...
                  <TableCell>
                    <Badge variant="secondary" className="bg-purple-100 text-purple-700 font-medium text-[10px] sm:text-xs">
                      {offer.status}
                    </Badge>
                  </TableCell>
