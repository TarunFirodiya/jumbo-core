...
import { deals } from "@/mock-data/deals"; // Reuse deals mock data for now or create new mock data if needed

export function BuyersTable() {
  // Simplified version without store dependency for now to ensure it works standalone
  const [currentPage, setCurrentPage] = React.useState(1);
  const pageSize = 10;
  
  // Reuse deals data but mapped as buyers
  const buyers = deals.map(d => ({
    id: d.id,
    name: d.client,
  ...
  const totalPages = Math.ceil(buyers.length / pageSize);
  const paginatedBuyers = buyers.slice((currentPage - 1) * pageSize, currentPage * pageSize);
          ...
          <TableBody>
            {paginatedBuyers.map((buyer, index) => (
              <TableRow key={buyer.id}>
                ...
                <TableCell>
                  <span className="font-medium text-xs sm:text-sm block truncate">{buyer.name}</span>
                </TableCell>
                ...
                <TableCell className="hidden lg:table-cell">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground text-xs sm:text-sm">{buyer.agent}</span>
                  </div>
                </TableCell>
                <TableCell>
                  <DropdownMenu>
