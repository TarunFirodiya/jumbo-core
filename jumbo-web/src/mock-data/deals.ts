...
function getRandomDate(seed: number, startMonth: number, endMonth: number): string {
  const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
  const randomMonth = getDeterministicRandom(seed);
  const randomDay = getDeterministicRandom(seed + 1);
  
  const month = Math.floor(randomMonth * (endMonth - startMonth + 1)) + startMonth;
...
export const deals: Deal[] = dealTemplates.map((template, index) => {
  ...
  const randomValue = getDeterministicRandom(index + 100);
  const value = Math.floor(randomValue * 40000) + 5000;
  ...
  return {
    ...
    expectedClose: getRandomDate(index, 3, 8),
  };
});
